...
def balanced(m):
    ...
    if left_torque != right_torque:
        ...
        if is_planet(left_end) and is_planet(right_end):
            return True
        elif is_mobile(left_end) and is_planet(right_end):
            return balanced(end(left_arm))
        elif is_planet(left_end) and is_mobile(right_end):
...
def multiple_references_explanation():
    return
    """
    By definition, the maping of operation add_interval and sub_interval is one-to-one,
    however, the mapping of mul_interval and div_interval is not one-to-one.In such a
    way, the latter two operations may cause multiple references.
    """
...
def quadratic(x, a, b, c):
    ...
    midpoint = - b / (2 * a)
    upper = upper_bound(x)
    value1 = a * lower * lower + b * lower + c
    value2 = a * midpoint * midpoint + b * midpoint + c
    value3 = a * upper * upper + b * upper + c
    if upper < midpoint or lower > midpoint:
        return interval(min(value1, value3), max(value1, value3))
    else:
