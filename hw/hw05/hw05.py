class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, product, cost, inventory = 0, curr_balance = 0):
    ...
    def vend(self):
        if self.inventory == 0:
            ...
            if self.balance < self.cost:
                return 'You must add $' + str(self.cost - self.balance) + ' more funds.'
        
            else:
                self.inventory -= 1
                remain = self.balance - self.cost
                self.balance = 0
                return 'Here is your ' + self.product + ('.' if remain == 0 else ' and $' + str(remain) + ' change.')
