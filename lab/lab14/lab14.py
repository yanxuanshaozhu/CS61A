...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def split_helper(s, total):
        if not s:
            return 1 if abs(total) <= d else 0
        first, second = s[0], s[1:]
        return split_helper(second, total + first) + split_helper(second, total - first)
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if check.payable_to == self.holder and check.deposited is False:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        edits, cost = "+[{}]".format(code_char) + edits, cost + 1
        possibilities.append((edits, cost))
        # Delete
        edits, cost = helper_align(skeleton_idx+1, code_idx)
        edits, cost = "-[{}]".format(skel_char) + edits, cost + 1
        possibilities.append((edits, cost))
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda link, y: Link(y, link), Link.empty)
