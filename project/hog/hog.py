...
def free_bacon(score):
    ...
    """
    A faster way:
    ls = [int(ele) for ele in list(str(pi))]
    return ls[score] + 3
    """
    a = pi // pow(10, 101 - score - 1)
    b = pi // pow(10, 101 - score)
    return a - 10 * b + 3
    # END PROBLEM 2
...
def announce_highest(who, last_score: int = 0, running_high: int = 0):
    ...
    def high(score0: int, score1: int):
        if who == 0:
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average(*args):
        ...
        for i in range(trials_count):
            rs += original_function(*args)
        return rs / trials_count
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    ls = []
    for i in range(1, 11):
        returns = make_averaged(roll_dice, trials_count)
        ls.append((returns(i, dice), i))
